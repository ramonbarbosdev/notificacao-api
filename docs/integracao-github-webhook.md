...
7. **Regras por coluna (board):** `dsGithubRegrasPorStatus` (JSON, migration V42). Chave de cada coluna = `optionId` do campo Status no Project v2 (GraphQL). Por coluna: `aoEntrar.fluxoGeral`, `aoEntrar.prAvaliadores`, `aoEntrar.issueAvaliadores`; opcional `destinatarios.modo` (`INHERIT` ou override) e `mensagem.cenarioId`. Ao salvar, a API espelha nomes nas listas legadas `dsGithubStatusDisparo`, `dsGithubPrStatusDisparo`, `dsGithubIssueStatusDisparo` para compatibilidade. No webhook, com JSON persistido, o fluxo geral/PR/Issue por coluna usa o mapa (match por nome normalizado de `changes.field_value.to.name`); sem JSON, vale o comportamento das listas em virgula.
8. Opcional (legado / espelho): `dsGithubStatusDisparo` (virgula) = colunas do board no **fluxo geral**. Quais gatilhos usam essa lista: `dsGithubStatusDisparoGatilhos` (virgula de `STATUS_ALTERADO`, `REORDENADO`, `RESPONSAVEL_ALTERADO`, etc.). **Padrao** se NULL: `STATUS_ALTERADO,REORDENADO`. Ex.: responsavel alterado so passa pelo filtro de colunas se `RESPONSAVEL_ALTERADO` estiver na lista (na UI: checkboxes em Regras, secao “Fora do board”).
